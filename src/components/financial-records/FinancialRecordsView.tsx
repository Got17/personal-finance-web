...
export function FinancialRecordsView({ initialRecords, accounts, categories }: Props) {
  ...
  return <div className={styles.view}>
    ...
    <div className={styles.list}>{visibleRecords.map((record) => <article key={record.id}><div><strong>{nameFor(record.category_id, categories)}</strong><p>{nameFor(record.account_id, accounts)} · {record.date.slice(0, 10)}</p>{record.note && <small>{record.note}</small>}</div><b className={record.kind === "income" ? styles.income : undefined}>{record.kind === "income" ? "+" : "−"}{money(record.amount_minor, record.currency)}</b></article>)}{visibleRecords.length === 0 && <p className={styles.empty}>No transactions match these filters.</p>}</div></section>
  </div>;
}
